...
export default {
    data: new SlashCommandBuilder()
        .setName('leaderboard')
        .setDescription('View server music listening leaderboard')
    ...
    async execute(interaction, client) {
        ...
        // This command is for SERVER leaderboard only
        const scope = 'server';
        ...
        for (const [userId, guilds] of Object.entries(stats.users || {})) {
            if (scope === 'server') {
                ...
                const guildStats = guilds[interaction.guild.id];
                if (guildStats) {
                    leaderboard.push({
                        userId,
                        tracks: guildStats.tracks || 0,
        ...
        if (userIndex !== -1) {
            ...
            if (type === 'tracks') {
                footer = `Your rank: #${userIndex + 1} with ${userEntry.tracks} tracks`;
            } else {
                const hours = Math.floor(userEntry.minutes / 60);
                const mins = userEntry.minutes % 60;
                footer = `Your rank: #${userIndex + 1} with ${hours}h ${mins}m`;
            }
        } else {
        ...
        const embed = new EmbedBuilder()
            ...
            .setTitle(`${client.config.emojis.stars} ${interaction.guild.name} Music Leaderboard`)
